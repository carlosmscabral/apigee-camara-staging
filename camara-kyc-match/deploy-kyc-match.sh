...
check_env_var KYC_MATCH_TARGET_SERVER_URI
check_env_var KYC_MATCH_TARGET_SERVER_PATH
...
sed -i 's|#PATH_PLACEHOLDER#|'"${KYC_MATCH_TARGET_SERVER_PATH}"'|g' ./apiproxy/targets/default.xml || { echo "Error: Could not update default.xml"; exit 1; }
...
apigeecli targetservers create --name ${TARGET_SERVER_NAME}  --org "$APIGEE_PROJECT_ID" --env "$APIGEE_ENV" --tls true --port 443 --host "$KYC_MATCH_TARGET_SERVER_URI" --token "$TOKEN" || { echo "Error: Could not create target server or it already exists. Proceeding with the setup..."; }
